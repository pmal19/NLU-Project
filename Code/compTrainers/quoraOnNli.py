...
class CompNetQuoraonNLI(nn.Module):
    def __init__(self, inp_dim, model_dim, num_layers, reverse, bidirectional, dropout, path1, training = True):
        super(CompNetQuoraonNLI, self).__init__()
        self.encoder1 = TaskEncoder(inp_dim, model_dim, num_layers, reverse, bidirectional, dropout, False, "", "", path1, "quora")
...
def trainEpoch(epoch, break_val, trainLoader, model, optimizer, criterion, inp_dim, batchSize, use_cuda, devLoader, devbatchSize):
    ...
    for batch_idx, (data, target) in enumerate(trainLoader):
        ...
        if batch_idx % 100 == 0:
            ...
            save(model, optimizer, loss, 'compTrainerQuoraonNLI.pth', dev_loss)
...
def main():
    ...
    model = CompNetQuoraonNLI(inp_dim, model_dim, num_layers, reverse, bidirectional, dropout, quora_path, training)
