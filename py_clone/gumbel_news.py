...
def train_epoch_progress(model, train_iter, loss_function, optimizer, text_field, label_field, epoch, USE_GPU):
    ...
    for batch in tqdm(train_iter, desc='Train epoch '+str(epoch+1)):
        sent, label = batch.TITLE, batch.CATEGORY
        if sent.shape[1] != 32:
            continue
        if USE_GPU:
...
def load_news(text_field, label_field, batch_size):
...
train_iter, dev_iter, test_iter = load_news(text_field, label_field, BATCH_SIZE)
# pdb.set_trace()
