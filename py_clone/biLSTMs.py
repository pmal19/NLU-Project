...
class GumbelQuora(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, label_size, use_gpu, batch_size, sst_path, nli_path, dropout=0.5):
        ...
        self.embeddings = nn.Embedding(vocab_size, embedding_dim)
        self.hidden2label = nn.Linear(hidden_dim*2, label_size)
        self.g_linear1=nn.Linear(hidden_dim*2, 2)
        self.hidden = self.init_hidden()
    ...
    def forward(self, sentence1, sentence2):
        ...
        g_inp=torch.cat((nli_out[-1], sst_out[-1]), 2)
        out_l=self.g_linear1(g_inp)
        out_l=F.relu(out_l)
        out_l=F.log_softmax(out_l)
        selector=self.st_gumbel_softmax(out_l)
        r1 = selector[:,0]
        r2 = selector[:,1]
        r11 = r1.repeat(self.hidden_dim,1)
        r22 = r2.repeat(self.hidden_dim,1)
