...
# class BiLSTMCompQuora(nn.Module):

#     def __init__(self, embedding_dim, hidden_dim, vocab_size, label_size, use_gpu, batch_size, dropout=0.5, targetTask="quora", sstPath, enc2Path):
#         super(BiLSTMDuplicate, self).__init__()
#         self.hidden_dim = hidden_dim
#         self.use_gpu = use_gpu
#         self.batch_size = batch_size
#         self.dropout = dropout
#         self.embeddings = nn.Embedding(vocab_size, embedding_dim)
#         self.lstmDuplicate1 = nn.LSTM(input_size=embedding_dim*2, hidden_size=hidden_dim, bidirectional=True)
#         self.hidden2label = nn.Linear(hidden_dim*2, label_size)
#         self.hidden = self.init_hidden()
...
#         x2 = self.embeddings(sentence2).view(len(sentence2), self.batch_size, -1)
#         x = torch.cat((x1, x2), 2)
#         lstm_out, self.hidden = self.lstmDuplicate1(x, self.hidden)
#         y = self.hidden2label(lstm_out[-1])
#         log_probs = F.log_softmax(y)
#         return log_probs