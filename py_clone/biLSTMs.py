...
class BiLSTMNews(nn.Module):

    def __init__(self, embedding_dim, hidden_dim, vocab_size, label_size, use_gpu, batch_size, dropout=0.5):
        super(BiLSTMNews, self).__init__()
        self.hidden_dim = hidden_dim
        self.use_gpu = use_gpu
        self.batch_size = batch_size
        self.dropout = dropout
    ...
    def forward(self, sentence):
        x = self.embeddings(sentence).view(len(sentence), self.batch_size, -1)
        lstm_out, self.hidden = self.lstmNews(x, self.hidden)
        y = self.hidden2label(lstm_out[-1])
        log_probs = F.log_softmax(y)
        return log_probs
...
class GumbelNLI(nn.Module):

    def __init__(self, embedding_dim, hidden_dim, vocab_size, label_size, use_gpu, batch_size, sst_path, quora_path, dropout=0.5):
        super(GumbelNLI, self).__init__()
    ...
    def forward(self, sentence1, sentence2):
        ...
        nli_out1 = self.lstmDuplicate(x1)
...
class GumbelSST(nn.Module):

    def __init__(self, embedding_dim, hidden_dim, vocab_size, label_size, use_gpu, batch_size, nli_path, quora_path, dropout=0.5):
        super(GumbelSST, self).__init__()
        ...
        self.embeddings = nn.Embedding(vocab_size, embedding_dim)
        self.hidden2label = nn.Linear(hidden_dim*4, label_size)
        self.g_linear1=nn.Linear(hidden_dim*4, 2)
        # self.hidden = self.init_hidden()
